...
class BasicModel(object):
    def __init__(self, **kwargs):
        ...
        self.permute = kwargs.get('permute', 1)
    ...
    def _build_net(self):
        ...
        with tf.variable_scope('actor') as scope:
            ...
            connected_emb = tf.stop_gradient(self.pred_s)
    ...
    def _build_loss(self):
        with tf.name_scope('forward'):
            ...
            loss_v = cls_loss + self.cali * cal_loss

            loss_z = self.lamb * mmd_loss_z  # - self.det_1

            tf.summary.scalar('cls_loss_1', cls_loss_1)
    ...
    def _build_opt(self):
        ...
        return adam.minimize(self.loss, self.global_step, var_list=var_list)
...
class ClsModel(BasicModel):
    ...
    def _cls_opt(self):
        cls_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=self.v_label, logits=self.cls_fc))
        tf.summary.scalar('cls/loss', cls_loss)
        adam = tf.train.AdamOptimizer(self.lr)
        var_list = tf.trainable_variables(scope='cls')
        opt = adam.minimize(cls_loss, global_step=self.global_step, var_list=var_list)
        return opt

    def train(self, restore_file=None, restore_list=None, task='hehe1', max_iter=500000):
        opt = self._build_opt()
        cls_opt = self._cls_opt()
        init_op = tf.global_variables_initializer()
        self.sess.run(init_op)
        time_string = strftime("%a%d%b%Y-%H%M%S", gmtime())
        summary_path = os.path.join('./result', self.set_name, 'log', task + '_' + time_string) + os.sep
        ...
        summary = tf.summary.merge(
            tf.get_collection(tf.GraphKeys.SUMMARIES, scope='forward') + tf.get_collection(tf.GraphKeys.SUMMARIES,
                                                                                           scope='reverse'))
        cls_summary = tf.summary.merge(tf.get_collection(tf.GraphKeys.SUMMARIES, scope='cls'))
        ...
        for i in range(max_iter):
            feed_dict = {self.data.train_test_handle: self.data.training_handle}
            values = self.sess.run([self.cls_fc, self.v_label, cls_opt, cls_summary, self.global_step],
                                   feed_dict=feed_dict)
            writer.add_summary(values[-2], values[-1])
            if (i + 1) % 50 == 0:
                seen_dict = {self.data.train_test_handle: self.data.seen_handle}
                unseen_dict = {self.data.train_test_handle: self.data.unseen_handle}

                seen_values = self.sess.run([self.cls_fc[:self.batch_size, :], self.label], feed_dict=seen_dict)
                unseen_values = self.sess.run([self.cls_fc[:self.batch_size, :], self.label], feed_dict=unseen_dict)

                train_acc = zsl_acc.cls_wise_prob_acc(values[0], values[1])
                seen_acc = zsl_acc.cls_wise_prob_acc(seen_values[0], seen_values[1])
                unseen_acc = zsl_acc.cls_wise_prob_acc(unseen_values[0], unseen_values[1])
                h_score = zsl_acc.h_score(seen_acc, unseen_acc)
                step_value = values[-1]
                ...
                hook_summary = tf.Summary(value=[tf.Summary.Value(tag='hook/h_score', simple_value=h_score)])
                writer.add_summary(hook_summary, step_value)
                print('Step {}, train acc {}'.format(step_value, train_acc))
