...
def batch_norm(opts, _input, is_train, reuse, scope, scale=True):
    ...
    """
    return tf.contrib.layers.batch_norm(_input, center=True, scale=scale, epsilon=1e-5, decay=0.9, is_training=is_train,
                                        reuse=reuse, updates_collections=None, scope=scope, fused=False)


def conv_normalization_layer(batch_data, train: tf.Tensor, beta=None, gamma=None):
    """
    ...
    with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
        ...
        ema = tf.train.ExponentialMovingAverage(decay=.99)
    ...
    if gamma is None:
        gamma = tf.Variable(tf.constant(1.0, shape=[n_out]), name='bn_gamma', trainable=True)

    return tf.nn.batch_normalization(batch_data, mean, var, beta, gamma, 1e-3)
...
def gumbel_trick(x: tf.Tensor, temp, stochastic=True):
